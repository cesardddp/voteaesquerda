...
import { ref } from "vue";
import { CreateDepEstadualInput, CreateGovEstadualInput, estados, GovEstadual } from "../API";
import { createGovEstadual } from "../graphql/mutations";

const estados_lista = ref(Object.values(estados).sort())
const input = ref<CreateGovEstadualInput>(
    {
        nome: "",
        foto: "",
        numero: null,
        ...
        saibaMais: "",
        entrevista: "",
        estadoGovEstadualId: estados.AC,
    }
)

const cria = async (): Promise<GovEstadual> => {
    const t = await (API.graphql({ query: createGovEstadual, variables: { input: input } }) as Promise<
        GraphQLResult<CreateGovEstadualInput>
    >);
    return t.data as GovEstadual
...
<template>
    <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Gov. Estado</h5>
    ...
    <div class="modal-body">
        ...
        <select class="form-select" v-model="input.estadoGovEstadualId">
