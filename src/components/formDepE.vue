...
import { ref } from "vue";
import { CreateDepEstadualInput, CreateEstadoMutation, DepEstadual, Estado, estados } from "../API";
import { createDepEstadual, createEstado } from "../graphql/mutations";

const estados_lista = ref(Object.values(estados).sort())
const input = ref<CreateDepEstadualInput>(
    {
        nome: "",
        foto: "",
        numero: null,
        ...
        saibaMais: "",
        entrevista: "",
        estadoDepEstadualId: estados.AC,
    }
)

const cria = async (): Promise<DepEstadual> => {
    const t = await (API.graphql({ query: createDepEstadual, variables: { input: input } }) as Promise<
        GraphQLResult<CreateDepEstadualInput>
    >);
    return t.data as DepEstadual
...
<template>
    <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Dep. Estadual</h5>
    ...
    <div class="modal-body">
        ...
        <select class="form-select" v-model="input.estadoDepEstadualId">
